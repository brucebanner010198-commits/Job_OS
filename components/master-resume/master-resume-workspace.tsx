...
import { ShieldAlert, LayoutList, FileText } from "lucide-react";
...
export function MasterResumeWorkspace({ facts }: { facts: ProfileEntry[] }) {
  const [viewMode, setViewMode] = useState<"formatted" | "ats">("formatted");
    ...
    <section className="mt-8 space-y-4">
      <div className="flex flex-wrap items-center justify-between gap-3 border-b border-border pb-3">
        ...
        <div className="flex items-center gap-1 rounded-lg border border-border bg-muted/30 p-1 text-xs">
          <Button
            size="sm"
            variant={viewMode === "formatted" ? "accent" : "ghost"}
          ...
          <Button
            size="sm"
            variant={viewMode === "ats" ? "accent" : "ghost"}
            className="h-7 gap-1.5 px-2.5 text-xs"
            onClick={() => setViewMode("ats")}
          >
            <FileText className="h-3.5 w-3.5" />
            ATS Linear Parser View
          </Button>
        </div>
      </div>
